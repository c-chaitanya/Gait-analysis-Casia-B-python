...
def load_training_validation_data(train_view=None, val_view=None, train_dir=None, val_dir=None):
    ...
    for id in human_id:
        for dir in train_dir:
            ...
            img_dir = "%s/%s/%s" % (config.project.casia_dataset_b_path, id, dir)
            if not os.path.exists(img_dir):
                logger.warning("%s train dir do not exist" % img_dir)
        for dir in val_dir:
            #img_dir = "%s/%s/%s/%s" % (config.project.casia_dataset_b_path, id, dir, val_view)
            img_dir = "%s/%s/%s" % (config.project.casia_dataset_b_path, id, dir)
            if not os.path.exists(img_dir):
                logger.warning("%s val dir do not exist" % img_dir)
...
##################comment everything below this line#########################33
